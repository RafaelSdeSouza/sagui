...
Segment only the valid spaxels, using exact Ward clustering for small cubes or sparse Ward for large photometric cubes.
:::
...
For large cubes, switch to the memory-aware sparse-Ward backend:
...
Optional acceleration packages for exact clustering and selected workflows:
...
seg_large <- segment_regions_large(x, Ncomp = 30, pretransform = "asinh", knn_k = 40)
```
