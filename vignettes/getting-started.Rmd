...
Sagui prepares a common multiband cube, constructs a spatial support, assigns
categorical regions, and measures their integrated SEDs.
...
This fixed-seed simulation mixes bulge, disc, and two knot profiles in nine
bands.
...
The simulated galaxy already uses `NA` outside its support, so no starlet mask
is needed.
...
figure_path <- if (is.null(input_file)) {
  system.file(
    "help", "figures", "sagui-first-run-synthetic-sed.png",
    package = "sagui"
  )
} else {
...
observations, or [Export Regional SEDs](exporting-regional-seds.html) to save
the results.
